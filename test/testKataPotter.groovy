...
class testKataPotter {
	...
	void FirstDiscountTest() {
		basket << [BookFactory.firstBook(), BookFactory.secondBook()]
		assertEquals(BOOK_PRICE * basket.books.size() * 0.95, basket.result, 0)
	}	
}
...
from the series, you get a 5% discount on those two books. If you buy 3 different books, 
you get a 10% discount. With 4 different books, you get a 20% discount. If you go the 
whole hog, and buy all 5, you get a huge 25% discount.
...
of 0 books, 1 book, 2 identical books, 2 different books� and it is not too difficult to 
work in small steps and gradually introduce complexity.
...
The trick is that you have to write some code that is intelligent enough to notice that 
two sets of four books is cheaper than a set of five and a set of three.

You will have to introduce a certain amount of clever optimization algorithm. But not too 
much! This problem does not require a fully fledged general purpose optimizer. Try to 
solve just this problem well in order to share it for everyone or even in the ??? . Trust 
that you can generalize and improve your solution if and when new requirements come along.
